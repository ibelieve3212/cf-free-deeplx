addEventListener('fetch', async (event) => {
  var targetURLs = []; 
  await fetch('https://raw.githubusercontent.com/ibelieve3212/serch_deeplx/main/success_result.txt')
  .then(response => response.text())
  .then(data => {
      targetURLs = data.split(',');  
      console.log(targetURLs);
  });
  event.respondWith(handleRequest(targetURLs, event.request));
});
